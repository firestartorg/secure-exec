...
- Remove template-literal-based generated source assembly from isolate injection paths.
...
3. Refactor runtime loaders (`index.ts`, `browser/worker.ts`, `bridge-loader.ts`) to consume compiled artifacts.
4. Remove template-literal code-generation paths for isolate injection and replace runtime value interpolation with structured config channels.
5. Run runtime tests, type checks, and build checks; update friction log with migration resolution notes.
...
- Should guardrails include a lint/test rule that fails on new template-literal-based isolate code assembly in host runtime files?
