...
function skipReason(): string | false {
	if (!existsSync(COMMANDS_DIR)) return 'WASM binaries not built (run make wasm in native/wasmvm/)';
	if (!existsSync(join(C_BUILD_DIR, 'tcp_server'))) return 'tcp_server not built (run make -C native/wasmvm/c sysroot && make -C native/wasmvm/c programs)';
	if (!existsSync(join(C_BUILD_DIR, 'http_get'))) return 'http_get not built (run make -C native/wasmvm/c programs)';
	return false;
}
...
describe.skipIf(skipReason())('cross-runtime network integration', { timeout: 30_000 }, () => {
	let kernel: Kernel;
